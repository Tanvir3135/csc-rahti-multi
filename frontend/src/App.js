...
function App() {
  ...
  const fetchDeveloperName = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/name`);
      setDeveloperName(response.data.name);
    } catch (error) {
  ...
  return (
    <div className="App">
      <header className="App-header">
        <h1>Tanvirs's Task Manager</h1>
        <p>Multi-container Demo App for CSC Rahti</p>

        {appInfo && (
          <div className="app-info">
            <small>
              Backend Version: {appInfo.version} | Environment:{" "}
              {appInfo.environment} | Container: {appInfo.container_id}
            </small>
          </div>
        )}
      </header>
